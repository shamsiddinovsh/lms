...
export default function Users() {
  ...
  const [count, setCount] = useState([1]);
    ...
    const handleDelete = (id) => {
      ...
      if (conf) {
        lang.delete('/delete/'+ id).then((res)=>{
          if (res.status === 200) {
            setCount(count + 1);
          }
        ...
        if(users){
            ...
            <div className="card-body">
              <div className="d-flex justify-content-between align-items-center">
                <h4 className="font-18 mb-3 text-muted font-weight-bold">{t("lang_study")}</h4>
                <NavLink
                  to={"/hr/study/lang/create"}
                    ...
                    {users.map((item, index) => (
                      <tr key={index}>
                        ...
                        <td className="text-center">
                          <NavLink to={`/hr/study/lang/${item.id}`} className="btn btn-warning" >
                          {t('update')}
                          </NavLink>
                        </td>
